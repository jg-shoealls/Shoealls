        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          ...
          {result && (
            <>
              ...
              {/* 이상 발견 */}
              {result.anomaly_findings && Object.keys(result.anomaly_findings).length > 0 && (
                <ResultCard title="이상 감지 결과" badge="Anomaly" accentColor={C.amber}>
                  <div className="space-y-3">
                    {Object.entries(result.anomaly_findings).map(([sensor, findings]) => (
                      <div key={sensor}>
                        <div className="text-textSec text-[12px] font-semibold mb-1 uppercase tracking-wide">
                          {sensor}
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {(findings as string[]).map((f) => (
                            <span
                              key={f}
