...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # mag_baro (dummy tensor if missing in schema)
    mag_baro_t = torch.zeros(1, data_cfg.get("mag_baro_channels", 5), seq_len, dtype=torch.float32)
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        for i in range(len(probs)):
            name = GAIT_CLASS_NAMES.get(i, (f"class_{i}", ""))[0]
