...
GAIT_CLASS_NAMES = {
    ...
    2: ("ataxic", "운동실조"),
    3: ("parkinsonian", "파킨슨"),
    4: ("class_4", "알 수 없는 질환(4)"),
    5: ("class_5", "알 수 없는 질환(5)"),
    6: ("class_6", "알 수 없는 질환(6)"),
    7: ("class_7", "알 수 없는 질환(7)"),
    8: ("class_8", "알 수 없는 질환(8)"),
    9: ("class_9", "알 수 없는 질환(9)"),
    10: ("class_10", "알 수 없는 질환(10)"),
}
...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # Inject dummy mag_baro tensor to avoid KeyError in forward pass
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        return GaitClassifyResponse(
            ...
            class_probabilities={
                GAIT_CLASS_NAMES.get(i, (f"class_{i}", ""))[0]: float(probs[i])
