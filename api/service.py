...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # Dummy mag_baro (e.g., shape 1, 5, seq_len) if missing to avoid KeyError
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        pred_en, pred_kr = GAIT_CLASS_NAMES[pred_idx] if pred_idx < len(GAIT_CLASS_NAMES) else (f"unknown_{pred_idx}", f"알수없음_{pred_idx}")
        ...
        return GaitClassifyResponse(
            ...
            class_probabilities={
                (GAIT_CLASS_NAMES[i][0] if i < len(GAIT_CLASS_NAMES) else f"unknown_{i}"): float(probs[i])
    ...
    def reasoning(self, sensor_data: SensorData, ckpt: str | None = None) -> ReasoningResponse:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            en, kr = GAIT_CLASS_NAMES[top_cls] if top_cls < len(GAIT_CLASS_NAMES) else (f"unknown_{top_cls}", f"알수없음_{top_cls}")
