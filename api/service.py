...
_CONFIG_PATH = Path(__file__).parent.parent / "configs" / "multitask.yaml"
...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # Mag_baro: mock for now if not present, MultimodalGaitNet needs it
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        pred_en, pred_kr = GAIT_CLASS_NAMES.get(pred_idx, (f"class_{pred_idx}", f"클래스 {pred_idx}"))
        ...
        return GaitClassifyResponse(
            ...
            class_probabilities={
                GAIT_CLASS_NAMES.get(i, (f"class_{i}", f"클래스 {i}"))[0]: float(probs[i])
    ...
    def reasoning(self, sensor_data: SensorData, ckpt: str | None = None) -> ReasoningResponse:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            en, kr = GAIT_CLASS_NAMES.get(top_cls, (f"class_{top_cls}", f"클래스 {top_cls}"))
