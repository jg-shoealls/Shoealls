...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    mag_baro_t = torch.zeros((1, mb_channels, seq_len), dtype=torch.float32)
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        pred_en, pred_kr = GAIT_CLASS_NAMES[min(pred_idx, len(GAIT_CLASS_NAMES)-1)]
        ...
        return GaitClassifyResponse(
            ...
            class_probabilities={
                GAIT_CLASS_NAMES[min(i, len(GAIT_CLASS_NAMES)-1)][0]: float(probs[i])
    ...
    def reasoning(self, sensor_data: SensorData, ckpt: str | None = None) -> ReasoningResponse:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            en, kr = GAIT_CLASS_NAMES[min(top_cls, len(GAIT_CLASS_NAMES)-1)]
