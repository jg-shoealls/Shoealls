...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # MagBaro: API input [T, 5] -> (5, T)
    mag_baro_t = None
    if hasattr(data, "mag_baro") and data.mag_baro:
        mb_np = np.array(data.mag_baro, dtype=np.float32)
        if mb_np.shape[0] < seq_len:
            pad = np.zeros((seq_len - mb_np.shape[0], mb_np.shape[1]), dtype=np.float32)
            mb_np = np.vstack([mb_np, pad])
        else:
            mb_np = mb_np[:seq_len]
        mb_np = mb_np.T # (5, T)
        mag_baro_t = torch.from_numpy(mb_np).unsqueeze(0)
    else:
        mag_baro_t = torch.zeros((1, 5, seq_len), dtype=torch.float32)
