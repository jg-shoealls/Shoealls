...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    res = {"imu": imu_t, "pressure": pressure_t, "skeleton": skeleton_t}
    mag_baro_channels = 5
    res["mag_baro"] = torch.zeros((1, mag_baro_channels, seq_len), dtype=torch.float32)
    return res
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        if pred_idx in GAIT_CLASS_NAMES:
            ...
            pred_en, pred_kr = (f"unknown_{pred_idx}", f"알수없음_{pred_idx}")

        class_probs = {}
        for i in range(len(probs)):
            name_en = GAIT_CLASS_NAMES[i][0] if i in GAIT_CLASS_NAMES else f"unknown_{i}"
            class_probs[name_en] = float(probs[i])
    ...
    def reasoning(self, sensor_data: SensorData, ckpt: str | None = None) -> ReasoningResponse:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            if top_cls in GAIT_CLASS_NAMES:
                ...
                en, kr = (f"unknown_{top_cls}", f"알수없음_{top_cls}")
