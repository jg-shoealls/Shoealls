...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # Inject dummy mag_baro tensor to satisfy MultimodalGaitNet KeyError
    mag_baro_t = torch.zeros(1, data_cfg.get("mag_baro_channels", 5), seq_len)
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        # Handle cases where dummy models output classes > 4 during testing
        pred_en, pred_kr = GAIT_CLASS_NAMES.get(pred_idx, (f"class_{pred_idx}", f"알 수 없는 클래스 ({pred_idx})"))

        class_probs = {}
        for i in range(len(probs)):
            en_name = GAIT_CLASS_NAMES.get(i, (f"class_{i}", ""))[0]
            class_probs[en_name] = float(probs[i])
    ...
    def reasoning(self, sensor_data: SensorData, ckpt: str | None = None) -> ReasoningResponse:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            en, kr = GAIT_CLASS_NAMES.get(top_cls, (f"class_{top_cls}", f"알 수 없는 클래스 ({top_cls})"))
