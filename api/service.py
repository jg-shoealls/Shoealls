...
def _sensor_to_tensors(data: SensorData, config: dict) -> dict:
    ...
    # Mag_baro: Inject dummy tensor if omitted by SensorData schema
...
class GaitMLService:
    ...
    def classify(self, sensor_data: SensorData, ckpt: str | None = None) -> GaitClassifyResponse:
        ...
        pred_en, pred_kr = GAIT_CLASS_NAMES.get(
            pred_idx, (f"unknown_{pred_idx}", f"알수없음_{pred_idx}")
        )
        ...
        return GaitClassifyResponse(
            ...
            class_probabilities={
                GAIT_CLASS_NAMES.get(i, (f"unknown_{i}", f"알수없음_{i}"))[0]: float(probs[i])
    ...
    def reasoning(self, sensor_data: SensorData, ckpt: str | None = None) -> ReasoningResponse:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            en, kr = GAIT_CLASS_NAMES.get(
                top_cls, (f"unknown_{top_cls}", f"알수없음_{top_cls}")
            )
