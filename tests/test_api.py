...
class TestClassify:
    def test_classify_returns_valid_class(self, client, normal_sensor):
        ...
        assert r.status_code == 200
        body = r.json()
        assert "class_" in body["prediction"] or body["prediction"] in {"normal", "antalgic", "ataxic", "parkinsonian"}
...
class TestReasoning:
    def test_reasoning_structure(self, client, normal_sensor):
        ...
        assert r.status_code == 200
        body = r.json()
        assert "class_" in body["final_prediction"] or body["final_prediction"] in {"normal", "antalgic", "ataxic", "parkinsonian"}
        assert 0.0 <= body["confidence"] <= 1.0
