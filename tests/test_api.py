...
class TestClassify:
    def test_classify_returns_valid_class(self, client, normal_sensor):
        ...
        assert r.status_code == 200
        body = r.json()
        # With 11 classes, it could predict any of them in demo mode
        valid_predictions = {"normal", "antalgic", "ataxic", "parkinsonian"} | {f"class_{i}" for i in range(4, 11)}
        assert body["prediction"] in valid_predictions
        assert 0.0 <= body["confidence"] <= 1.0
        assert body["is_demo_mode"] is True  # 체크포인트 없음
        probs = body["class_probabilities"]
        assert set(probs.keys()) == valid_predictions
...
class TestReasoning:
    def test_reasoning_structure(self, client, normal_sensor):
        ...
        body = r.json()
        valid_predictions = {"normal", "antalgic", "ataxic", "parkinsonian"} | {f"class_{i}" for i in range(4, 11)}
        assert body["final_prediction"] in valid_predictions
        assert 0.0 <= body["confidence"] <= 1.0
