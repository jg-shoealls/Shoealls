...
class TestClassify:
    def test_classify_returns_valid_class(self, client, normal_sensor):
        ...
        assert r.status_code == 200
        body = r.json()
        valid_classes = {"normal", "parkinsons", "stroke", "diabetic_neuropathy", "cerebellar_ataxia", "osteoarthritis", "dementia", "cerebral_hemorrhage", "cerebral_infarction", "disc_herniation", "rheumatoid_arthritis"}
...
class TestReasoning:
    def test_reasoning_structure(self, client, normal_sensor):
        ...
        body = r.json()
        valid_classes = {"normal", "parkinsons", "stroke", "diabetic_neuropathy", "cerebellar_ataxia", "osteoarthritis", "dementia", "cerebral_hemorrhage", "cerebral_infarction", "disc_herniation", "rheumatoid_arthritis"}
        assert body["final_prediction"] in valid_classes
        assert 0.0 <= body["confidence"] <= 1.0
