...
class TestClassify:
    def test_classify_returns_valid_class(self, client, normal_sensor):
        ...
        assert r.status_code == 200
        body = r.json()
        assert body["prediction"] in {"normal", "antalgic", "ataxic", "parkinsonian", "hemiplegic", "neuropathic", "diplegic", "myopathic", "choreic", "sensory", "vestibular"}
        assert 0.0 <= body["confidence"] <= 1.0
        assert body["is_demo_mode"] is True  # 체크포인트 없음
        probs = body["class_probabilities"]
        assert set(probs.keys()) == {"normal", "antalgic", "ataxic", "parkinsonian", "hemiplegic", "neuropathic", "diplegic", "myopathic", "choreic", "sensory", "vestibular"}
...
class TestReasoning:
    def test_reasoning_structure(self, client, normal_sensor):
        ...
        assert r.status_code == 200
        body = r.json()
        assert body["final_prediction"] in {"normal", "antalgic", "ataxic", "parkinsonian", "hemiplegic", "neuropathic", "diplegic", "myopathic", "choreic", "sensory", "vestibular"}
        assert 0.0 <= body["confidence"] <= 1.0
