...
class WearGaitDataset(Dataset):
    ...
    def __init__(self, imu_windows: list[np.ndarray], pressure_windows: list[np.ndarray], labels: list[int], biomarker_vectors: list[np.ndarray] = None):
        self.imu_windows = imu_windows
        self.pressure_windows = pressure_windows
        self.labels = labels
        self.biomarker_vectors = biomarker_vectors
        self.biomarker_vectors = biomarker_vectors
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.biomarker_vectors is not None:
            item["biomarkers"] = torch.as_tensor(self.biomarker_vectors[idx], dtype=torch.float32)
        return item
