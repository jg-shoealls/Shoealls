...
class CrossModalAttentionFusion(nn.Module):
    ...
    def forward(self, modality_features: list[torch.Tensor]) -> torch.Tensor:
        ...
        attn_out, _ = self.self_attention(combined, combined, combined, need_weights=False)  # BOLT OPTIMIZATION: need_weights=False enables fastpath attention and avoids allocating memory for discarded weights
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, query: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        attn_out, _ = self.cross_attn(query, context, context, need_weights=False)  # BOLT OPTIMIZATION: need_weights=False enables fastpath attention and avoids allocating memory for discarded weights
