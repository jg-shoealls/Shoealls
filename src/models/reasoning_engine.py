...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Optimization: Set need_weights=False to prevent unnecessary computation and memory allocation
        # since cross_attn_weights are discarded. Unpacking target renamed to _ to indicate it's unused.
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        cls_name = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"알수없음_{pred}"
        lines.append(f"  최종 판정: {cls_name}")
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            top_cls_name = self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else f"알수없음_{top_cls}"
            lines.append(
                f"  {label}: "
        ...
        for cls_idx in ranked:
            ...
            cls_name = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"알수없음_{cls_idx}"
            lines.append(
