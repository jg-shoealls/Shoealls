...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Optimization: need_weights=False skips returning attention weights for better performance. Target variable renamed to _ as it is unused.
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        for cls_idx in ranked:
            ...
            cls_idx_kr = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"알수없음_{cls_idx}"
            lines.append(
                f"  {marker} {cls_idx_kr:10s} "
        ...
        for s in range(1, len(trace)):
            ...
            if prev_top != curr_top:
                prev_top_kr = self.CLASS_NAMES_KR[prev_top] if prev_top < len(self.CLASS_NAMES_KR) else f"알수없음_{prev_top}"
                curr_top_kr = self.CLASS_NAMES_KR[curr_top] if curr_top < len(self.CLASS_NAMES_KR) else f"알수없음_{curr_top}"
                changes.append(
                    f"  단계{s}: {prev_top_kr} → {curr_top_kr}"
                )
