...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Set need_weights=False to save memory/computation and enable FlashAttention
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        pred_name = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"알수없음_{pred}"
        lines.append(f"  최종 판정: {pred_name}")
