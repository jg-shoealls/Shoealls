...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Optimization: cross_attn_weights are completely unused downstream.
        # Unpacking target renamed to _ and need_weights=False added to prevent
        # unnecessary computation/memory allocation and enable optimized attention backends.
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        lines.append(f"  최종 판정: {self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"알수없음_{pred}"}")
