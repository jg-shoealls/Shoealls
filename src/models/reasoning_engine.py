...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        cross_out, _ = self.cross_verify(
            summaries, summaries, summaries, need_weights=False,  # BOLT OPTIMIZATION: need_weights=False enables fastpath attention and avoids allocating memory for discarded weights
        )
