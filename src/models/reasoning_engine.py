...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # need_weights=False prevents memory allocation for attention weights and enables FlashAttention
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        pred_kr = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"클래스_{pred}"
        lines.append("")
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            top_kr = self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else f"클래스_{top_cls}"
            label = "초기 가설" if step_idx == 0 else f"추론 {step_idx}단계"
            lines.append(
                f"  {label}: "
                f"{top_kr} ({step_probs[top_cls]:.0%})"
            )
        ...
        for cls_idx in ranked:
            ...
            cls_kr = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"클래스_{cls_idx}"
            lines.append(
                f"  {marker} {cls_kr:10s} "
        ...
        for s in range(1, len(trace)):
            ...
            if prev_top != curr_top:
                prev_kr = self.CLASS_NAMES_KR[prev_top] if prev_top < len(self.CLASS_NAMES_KR) else f"클래스_{prev_top}"
                curr_kr = self.CLASS_NAMES_KR[curr_top] if curr_top < len(self.CLASS_NAMES_KR) else f"클래스_{curr_top}"
                changes.append(
