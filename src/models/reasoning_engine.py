...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Optimized: need_weights=False prevents unnecessary computation and memory allocation when weights are discarded
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        fallback_name = f"알수없음_{pred}"
        class_name = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else fallback_name
