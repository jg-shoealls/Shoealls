...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        def _safe_class_name(idx):
            if 0 <= idx < len(self.CLASS_NAMES_KR):
                return self.CLASS_NAMES_KR[idx]
            return f"알 수 없음 ({idx})"

        # ── 최종 판정 ──
        lines.append("")
        lines.append(f"  최종 판정: {_safe_class_name(pred)}")
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            lines.append(
                f"  {label}: "
                f"{_safe_class_name(top_cls)} ({step_probs[top_cls]:.0%})"
            )
        ...
        for cls_idx in ranked:
            ...
            lines.append(
                f"  {marker} {_safe_class_name(cls_idx):10s} "
        ...
        for s in range(1, len(trace)):
            ...
            if prev_top != curr_top:
                changes.append(
                    f"  단계{s}: {_safe_class_name(prev_top)} → {_safe_class_name(curr_top)}"
                )
