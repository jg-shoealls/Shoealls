...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Optimization: Set need_weights=False to prevent unnecessary computation
        # and memory allocation since attention weights are completely unused downstream.
        # Safely renaming the unpacking target to _
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        class_name = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"알수없음_{pred}"
        lines.append(f"  최종 판정: {class_name}")
        confidence = probs[pred] if pred < len(probs) else probs.max()
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            class_name = self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else f"알수없음_{top_cls}"
            lines.append(
                f"  {label}: "
                f"{class_name} ({step_probs[top_cls]:.0%})"
            )
        ...
        for cls_idx in ranked:
            ...
            class_name = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"알수없음_{cls_idx}"
            lines.append(
                f"  {marker} {class_name:10s} "
        ...
        for s in range(1, len(trace)):
            ...
            if prev_top != curr_top:
                prev_name = self.CLASS_NAMES_KR[prev_top] if prev_top < len(self.CLASS_NAMES_KR) else f"알수없음_{prev_top}"
                curr_name = self.CLASS_NAMES_KR[curr_top] if curr_top < len(self.CLASS_NAMES_KR) else f"알수없음_{curr_top}"
                changes.append(
