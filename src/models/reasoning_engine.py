...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # ⚡ Bolt: need_weights=False to save memory and enable optimized attention backends (e.g. FlashAttention)
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # Get safely named classes in case prediction index is out of bounds
        def get_class_name(idx):
            return self.CLASS_NAMES_KR[idx] if idx < len(self.CLASS_NAMES_KR) else f"알 수 없는 질환({idx})"
