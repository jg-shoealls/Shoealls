...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # ⚡ Bolt: Set need_weights=False to skip attention weight allocation
        # This prevents quadratic memory overhead and enables optimized backends (FlashAttention).
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        pred_kr = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"클래스 {pred}"
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            top_cls_kr = self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else f"클래스 {top_cls}"
            lines.append(
                f"  {label}: "
        ...
        for cls_idx in ranked:
            ...
            cls_kr = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"클래스 {cls_idx}"
            lines.append(
                f"  {marker} {cls_kr:10s} "
        ...
        for s in range(1, len(trace)):
            ...
            if prev_top != curr_top:
                prev_kr = self.CLASS_NAMES_KR[prev_top] if prev_top < len(self.CLASS_NAMES_KR) else f"클래스 {prev_top}"
                curr_kr = self.CLASS_NAMES_KR[curr_top] if curr_top < len(self.CLASS_NAMES_KR) else f"클래스 {curr_top}"
                changes.append(
