...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # Optimization: set need_weights=False to prevent unnecessary computation and
        # enable optimized attention backends (like FlashAttention) since weights are unused.
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        pred_kr_name = self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else f"알수없음_{pred}"
        lines.append(f"  최종 판정: {pred_kr_name}")
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            top_cls_kr_name = self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else f"알수없음_{top_cls}"
            lines.append(
                f"  {label}: "
                f"{top_cls_kr_name} ({step_probs[top_cls]:.0%})"
            )
        ...
        for cls_idx in ranked:
            ...
            cls_kr_name = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"알수없음_{cls_idx}"
            lines.append(
                f"  {marker} {cls_kr_name:10s} "
