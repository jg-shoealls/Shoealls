...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # need_weights=False prevents unnecessary computation and memory allocation, enabling optimized attention backends (like FlashAttention) when attention weights are discarded.
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            top_kr = self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else f"알수없음_{top_cls}"
            lines.append(
                f"  {label}: "
        ...
        for cls_idx in ranked:
            ...
            cls_kr = self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else f"알수없음_{cls_idx}"
            lines.append(
                f"  {marker} {cls_kr:10s} "
