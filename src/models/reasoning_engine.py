...
class CrossModalEvidenceCollector(nn.Module):
    ...
    def forward(
        ...
        # ⚡ Optimization: Set need_weights=False since attention weights are unused,
        # enabling optimized attention backends and saving memory/computation.
...
class GaitReasoningEngine(nn.Module):
    ...
    def explain(self, result: dict, sample_idx: int = 0) -> str:
        ...
        # ── 최종 판정 ──
        lines.append("")
        lines.append(f"  최종 판정: {self.CLASS_NAMES_KR[pred] if pred < len(self.CLASS_NAMES_KR) else '알수없음_' + str(pred)}")
        ...
        for step_idx, step_logits in enumerate(trace):
            ...
            lines.append(
                f"  {label}: "
                f"{self.CLASS_NAMES_KR[top_cls] if top_cls < len(self.CLASS_NAMES_KR) else '알수없음_' + str(top_cls)} ({step_probs[top_cls]:.0%})"
            )
        ...
        for cls_idx in ranked:
            ...
            lines.append(
                f"  {marker} {self.CLASS_NAMES_KR[cls_idx] if cls_idx < len(self.CLASS_NAMES_KR) else '알수없음_' + str(cls_idx):10s} "
        ...
        for s in range(1, len(trace)):
            ...
            if prev_top != curr_top:
                changes.append(
                    f"  단계{s}: {self.CLASS_NAMES_KR[prev_top] if prev_top < len(self.CLASS_NAMES_KR) else '알수없음_' + str(prev_top)} → {self.CLASS_NAMES_KR[curr_top] if curr_top < len(self.CLASS_NAMES_KR) else '알수없음_' + str(curr_top)}"
                )
