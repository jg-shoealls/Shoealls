...
class MultimodalGaitNet(nn.Module):
    ...
    def extract_features(self, batch: dict) -> torch.Tensor:
        ...
        # Handle optional mag_baro data gracefully
        if "mag_baro" in batch:
            mb_feat = self.mag_baro_encoder(batch["mag_baro"])
        else:
            # (Batch, Channels, SeqLen) = (B, 5, T)
            mb_feat = self.mag_baro_encoder(torch.zeros(batch["imu"].size(0), 5, batch["imu"].size(2), device=batch["imu"].device))
