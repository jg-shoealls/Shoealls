...
def plot_gait_profile_deviation(
    ...
    plot_metrics = []
    plot_labels = []
    _EXTRA_KR_LOCAL = {"injury_risk": "부상 위험도", "overall_deviation": "개인 기준 편차"}

    for m in metrics:
        if m in session_features and m in baseline_means:
            plot_metrics.append(m)
            plot_labels.append(_EXTRA_KR_LOCAL.get(m, get_feature_korean(m)))
...
def plot_trend_dashboard(
    ...
    for idx, metric in enumerate(available):
        ...
        # Annotation
        _METRIC_KR_LOCAL = {"injury_risk": "부상 위험도", "overall_deviation": "개인 기준 편차"}
        kr_name = _METRIC_KR_LOCAL.get(metric, get_feature_korean(metric))
